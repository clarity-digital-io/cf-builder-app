      ...
      StatusHandler(
        ...
        () =>
          call(
            ...
            [
              JSON.stringify(preppedConnections),
              form.Id,
              JSON.stringify(preppedRemovedConnections),
            ],
            (result, e) =>
              connectionsResultHandler(result, e, setConnections, setUpdate),
            form.forms__Status__c
          ),
        null,
        setError
  ...
  const add = () => {
    setConnections((connections) => {
      return connections.concat([
        { forms__Form__c: form.Id, forms__Salesforce_Object__c: "" },
      ]);
    });
  };
  ...
  return [
    <View className="row middle-xs end-xs" key={"Header"}>
      <View className="col-xs-12">
        <ViewStyle border>
          <Button onClick={() => setUpdate(true)}>Save Changes</Button>
        </ViewStyle>
      </View>
    </View>,
    <View body key={"Body"}>
      <Box padding="0">
        <ViewStyle space border>
          ...
          <p>
            Create new or update existing Salesforce records from Clarity Form
            Responses. Order determines which record is created first, giving
            you the ability to store and use a created Record Id.
          </p>
            ...
            return (
              <View key={connection.Id} space className="row middle-xs">
                ...
                <View className="col-xs-4">
                  <Box padding=".5em">
                    <Select
                      order={order}
                      options={sObjects}
                      value={connection.forms__Salesforce_Object__c}
                ...
                <View className="col-xs-2">
                  <Box padding=".5em">
                    {connection.Id != null ? (
                      <Button add onClick={() => edit(connection)}>
                        Edit
                      </Button>
                    ) : null}
                  </Box>
                ...
                <View className="col-xs-1">
                  <Box padding=".5em">
                    <div
                      style={closeStyle}
                      onClick={() => removeRow(order, connection)}
                    >
                      <CloseIcon />
                    </div>
                  </Box>
