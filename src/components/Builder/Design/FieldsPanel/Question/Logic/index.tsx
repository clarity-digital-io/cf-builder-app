...
export const LogicQuestion = () => {
  ...
  const { activeQuestion, questions, setActiveQuestion } =
    useContext(DesignContext);

  const [questionOptions, setQuestionOptions] = useState(
    questions.filter(
      (question) =>
        question.forms__Title__c != activeQuestion.forms__Title__c &&
        question.forms__Type__c != "RecordGroup" &&
        question.forms__Type__c != "FreeText" &&
        question.forms__Type__c != "PageBreak"
    )
  );
      ...
      <View className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <Box>
          ...
          {loading ? (
            ...
            <ViewStyle space top border>
              <h1>
                Step 1: <span>Select the criteria for this rule</span>
              </h1>

              <ControlGroup
                type="default"
                relatedId={activeQuestion.Id}
                value={activeQuestion.forms__Logic__c}
                rows={criteria}
                setRows={setCriteria}
